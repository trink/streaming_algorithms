...
static double pcc(sa_matrix_int *m, int r, int r1,
...
static sa_running_stats getrs(sa_matrix_int *m, int row)
{
  sa_running_stats rs;
...
static int matrix_pcc_int(lua_State *lua)
{
  ...
  double d = -INFINITY;
  int idx = -1;
  sa_running_stats rs = getrs(m, row);
  switch (match) {
  case 1:
    d = INFINITY;
    for (int i = 0; i < m->rows; ++i) {
      if (i == row) {continue;}
      sa_running_stats rs1 = getrs(m, i);
      double tmp = pcc(m, row, i, &rs, &rs1);
      if (tmp < d) {
        d = tmp;
        idx = i;
      }
